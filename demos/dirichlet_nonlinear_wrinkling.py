...
if __name__ == "__main__":
    ...
    # material properties for the substrate
    E_s = 10.
    nu_s = 0.475            # hopefully this doesn't lock on me...also could a bad quad rule cause no buckling?
    mu_s = E_s/2/(1+nu_s)
    lmbda_s = E_s*nu_s/(1+nu_s)/(1-2*nu_s)
    ...
    # model params for displacement stepping
    f_0 = 0.0 # known to work (at least with old model params...)
    ...
    delta_f_init = -0.005 # needs to decrease!
    delta_f = delta_f_init
    f_step_init = f_0                # initialize the load step
    f_step = f_step_init
    # f_max will be fixed
    f_max = -0.1                  # should really be looking at critical strain here...
    # one question: why am I struggling to converge with quads vs. linear elements? is my criteria off bc I have
    # such a large system?...
    ...
    # this might not be deep and wide enough; I think my critical wavelength is too close to 1...
    # a real/better TODO: solve this same problem in 2D...
    Lx, Ly, Lz = 5.0, 1.0, 5.0
    ...
    for n in refinements:
        ...
        for problem_type in problem_list:
            for deg in degree_list:
                ...
                bc1 = [[bottom], [2], [u_0]]  
                bc2 = [[top], [2], [u_0]]  
                bc3 = [[front_left]*3, [0,1,2], [u_0]*3]
                bc4 = [[left], [0], [u_0]]
                bc_disp = [[right]*3, [0,1,2], [u_disp_x, u_0, u_0]]

                #bc3 = [[left]*3,[0,1,2],[u_0]*3]
                dirichlet_bc_info = [[bc1, bc2, bc3, bc4, bc_disp]]
                ...
                if ADAPTIVE_LOAD_STEP:
                    ...
                    while (f_step > f_max) and (n_bisections < 4):
                        ...
                        # this accidentally jumps straight to the end lol
                        if f_step < f_max:
                            f_step = f_max    

                        # re-define the problem and solver to take another step...
                        def u_disp_x(point):
                            return f_step
                        ...
                        # define the solver
                        solver = Newton_Solver(problem, sol, line_search_flag=False)
                        ...
                        sol_file = data_dir + "/film_substrate_mesh_f_"+ str(f_max) + "_" + str(problem_type) + "_" + str(deg) + '_t_' + str(h_s) + '_disp_' + str(f_step) + '.vtu'
                        
                        # saving solution for each successive displacement
                ...
                sol_file = data_dir + "/film_substrate_mesh_f_"+ str(f_max) + "_" + str(problem_type) + "_" + str(deg) + '_t_' + str(h_s) + '.vtu'
