    ...
    for n in refinements:
        ...
        for problem_type in problem_list:
            for deg in degree_list:
                ...
                # # quickly access the cells in the film only
                # film_cells = get_film_cells(fe, film_substrate_obj)
                # print(film_cells)
                # onp.savetxt("film_cells_1120_562_u_0.01.txt", film_cells)

                # # load the solution in:
                # # would be a lot better to do this in a separate file
                # sol_file = data_dir + "/" + CASE + "_Nx_" + str(Nx_n) + "_Ny_" + str(Ny_n) + "_Nc_" + str(N_c) + "_u_" + str(u_max) + '.vtu'
                
                # mesh = meshio.read(sol_file)

                # new_cells = {'quad': mesh.cells_dict['quad'][onp.array(film_cells)]}
                # # mesh.cells_dict = new_cells
                # film_mesh = meshio.Mesh(fe.points, cells=new_cells)
